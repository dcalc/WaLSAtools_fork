...
IF silent EQ 0 THEN BEGIN
    ...
    IF EPS eq 1 THEN BEGIN
		...
		positioncb=[p1_x1,p1_y2+0.08,p1_x2,p1_y2+0.094]
    ENDIF ELSE BEGIN
    ...
    walsa_pg_plotimage_komega, komegamap, kopower_xscale[1:*], kopower_yscale[1:*], noy2=noy2, nox2=nox2, smooth=smooth, $
        xtitle='Wavenumber (pixel!U-1!N)', ytitle='Frequency (mHz)', xst=8, yst=8, xlog=xlog, ylog=ylog, position=[p1_x1, p1_y1, p1_x2, p1_y2], $
...
;         position=positioncb, /right, ticknames=tickmarknames, xticklen=0.00001, charsize=2.4, /vertical
	cgcolorbar, bottom=0, ncolors=255, divisions=3, minrange=MIN(kopower_plot[1:*,1:*],/nan), maxrange=MAX(kopower_plot[1:*,1:*],/nan), $
    	position=positioncb, /top, ticknames=tickmarknames, yticklen=0.00001, charsize=2.1
	
	xyouts, p1_x1+(p1_x2-p1_x1)/2., p1_y2+0.123, ALIGNMENT=0.5, CHARSIZE=1.1, /normal, 'Log!d10!n(Oscillation Power)', color=cgColor('Black')
	
	xyouts, p1_x1+(p1_x2-p1_x1)/2., p1_y2+0.16 , ALIGNMENT=0.5, CHARSIZE=1.3, /normal, '(a) k-'+cgGreek('omega')+' diagram and filtering', color=cgColor('Black')
...
; ++++++++++++++++++++++++++++++++++++++++++++++++++++++ k-omega Filtered Images
pos = cgLayout([6,6], OXMargin=[0,0], OYMargin=[0,0], XGap=2, YGap=2)
...
for i=0L, 5 do begin
	tmppos = reform(pos[*,i])
	position = [tmppos[0],imy1,tmppos[2],imy2]
	im = reform(filtered_cube[*,*,i])
	
	rgb = sjreadtext('Python_parameters/jet_colormap.txt') ; the jet colormap of Python!
	R = reform(rgb[0,*])*255 & G = reform(rgb[1,*])*255 & B = reform(rgb[2,*])*255
	tvlct, R, G, B
	
	walsa_image_plot, im, xrange=[0,129], yrange=[0,129], nobar=1, zrange=minmax(im), /nocolor, $
             contour=0, exact=1, aspect=1, cutaspect=1, barpos=1, noxval=1, distbar=80, $
			 noyval=1, cblog=cblog, position=position, xminor=2, yminor=2
	cgPlotS, 20, 109, PSym=16, SymColor='Black', SymSize=3.2, /data
	cgPlotS, 20, 109, PSym=16, SymColor='White', SymSize=2.7, /data
	cgtext, 20, 102., ALIGNMENT=0.5, CHARSIZE=1.1, /data, strtrim(long(i+1.),2), color=cgColor('Black')
endfor

xyouts, 0.5, 0.47 , ALIGNMENT=0.5, CHARSIZE=1.3, /normal, '(c) k-'+cgGreek('omega')+' filtered images', color=cgColor('Black')

; ++++++++++++++++++++++++++++++++++++++++++++++++++++++ POD Frequency Filtered Spatial Modes
...
for i=0L, 5 do begin
	tmppos = reform(pos[*,i])
	position = [tmppos[0],podffsmy1,tmppos[2],podffsmy2]
	
	podffsm = readfits('Python_parameters/pod_frequency_filtered_spatial_modes_frame'+strtrim(long(i),2)+'.fits')
...
xyouts, 0.5, 0.27 , ALIGNMENT=0.5, CHARSIZE=1.3, /normal, '(d) POD frequency-filtered spatial images', color=cgColor('Black')
...
pod_modes = readfits('Python_parameters/pod_first_6_spatial_modes.fits')

!x.ticklen=-0.058
!y.ticklen=-0.058

for i=0L, 5 do begin
	im = reform(pod_modes[*,*,i])
	rgb = sjreadtext('Python_parameters/jet_colormap.txt') ; the jet colormap of Python!
	R = reform(rgb[0,*])*255 & G = reform(rgb[1,*])*255 & B = reform(rgb[2,*])*255
	tvlct, R, G, B
	walsa_image_plot, im, xrange=[0,129], yrange=[0,129], nobar=1, zrange=minmax(im), /nocolor, $
	...
	cgPlotS, 20, 109, PSym=16, SymColor='Black', SymSize=3.2, /data
	cgPlotS, 20, 109, PSym=16, SymColor='White', SymSize=2.7, /data
	cgtext, 20, 102., ALIGNMENT=0.5, CHARSIZE=1.05, /data, '!5P!X'+strtrim(long(i+1.),2), color=cgColor('Black')
endfor

xyouts, xl+(xr-xl)/2., p1_y2+0.16, ALIGNMENT=0.5, CHARSIZE=1.3, /normal, '(b) POD spatial modes', color=cgColor('Black')

; ++++++++++++++++++++++++++++++++++++++++++++++++++++++ SPOD Spatial Modes
; Unnecessary to show them here! The pair SPOD modes are part of the POD frequency-filtered modes
; pos = fltarr(4,6)
; xl = 0.73 & xr = 0.99
; yb = 0.11 & yt = 0.54
; xgap = 0.
...
;
; for i=0L, 1 do begin
; 	spod = readfits('Python_parameters/spod_first_20_modes.fits')
;
; 	if i eq 0 then im = reform(spod[*,*,10])
; 	if i eq 1 then im = reform(spod[*,*,11])
; 	rgb = sjreadtext('Python_parameters/jet_colormap.txt') ; the jet colormap of Python!
; 	R = reform(rgb[0,*])*255 & G = reform(rgb[1,*])*255 & B = reform(rgb[2,*])*255
; 	tvlct, R, G, B
; 	walsa_image_plot, im, xrange=xrg, yrange=yrg, nobar=1, zrange=minmax(im), /nocolor, $
;              contour=0, exact=1, aspect=1, cutaspect=1, noxval=1, $
; 			 noyval=1, cblog=cblog, position=pos[*,i], xminor=2, yminor=2
; endfor
; ++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
walsa_endeps, filename='Figures/k-omega_and_pod_analyses'

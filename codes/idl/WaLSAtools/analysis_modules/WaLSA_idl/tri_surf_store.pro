; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/tri_surf.pro#1 $
;
; Copyright (c) 1993-2006, Research Systems, Inc.  All rights reserved.
;	Unauthorized reproduction prohibited.

;routine changed to store triangulation and reuse it when x and y are
;the same
FUNCTION TRI_SURF_STORE, z, x, y, REGULAR = regular, XGRID=xgrid, $
                         XVALUES = xvalues, YGRID = ygrid, YVALUES = yvalues, $
                         GS = gs, BOUNDS = bounds, NX = nx0, NY = ny0, $
                         MISSING = missing, EXTRAPOLATE = extrapolate, $
                         LINEAR=linear
  
  common tri_surf_store,zsize,xstore,ystore,tr, b
;+
; NAME:
;	TRI_SURF
;
...
;-
  ...
  if n_elements(zsize) eq 0 then redo=1 $
  else redo=(min(zsize eq s) and min(xstore eq x) and min(ystore eq y)) eq 0

pro sjim, image, cc=cc, win=win, cb=cb, clbcolor=clbcolor, clt=clt, $
		    ...
		    tif=tif, eps=eps, sizeeps=sizeeps, mac=mac, clockcolor = clockcolor, center=center, sideclip=sideclip, xrange=xrange, yrange=yrange, nowindow=nowindow
...
endelse
 ...
 if n_elements(nowindow) eq 0 then nowindow=0 else nowindow=1
 ...
 ; if n_elements(exact) eq 0 then actual=0 else actual=1
 ...
 if n_elements(mv) eq 0 then mv=0 else mv=1
 if mv eq 1 and n_elements(mcube) eq 0 then amcube=1
	...
	if n_elements(crispex) eq 2 then begin
		...
		if WLi eq -1 then begin
			...
			for mk=fr[0], fr[1] do begin
				im = lp_get(f[ifim],mk)
				im = smooth(im,smt,/EDGE_TRUNCATE)
				if aclip then im = im[clip[0]:clip[1],clip[2]:clip[3]]
				if actual then tvscl, congrid(iris_histo_opt(im),nxt/xcc,nyt/ycc), /nan else $
					tvscl, congrid(iris_histo_opt(im),nxt/xcc,nyt/ycc, /INTERP, /CENTER, cubic=-0.5), /nan 
	   	 	 	if cb then cgColorbar, Range=[min(im),max(im)], /vertical, color=clbcolor
				if ocontour eq 1 then begin
					contour, mask,/noerase,xst=5,yst=5,zst=5,c_color=cgColor('White'), c_thick=4, c_linestyle=0, NLEVELS=1
					contour, mask,/noerase,xst=5,yst=5,zst=5,c_color=cgColor('Black'), c_thick=1, c_linestyle=0, NLEVELS=1
				endif
	   		 	wait, 1./fps
 ...
 image = smooth(image,smt,/EDGE_TRUNCATE)
 ...
 if not nohist then begin
	 ...
	 if not actual then if not zm then tvscl, congrid(iris_histo_opt(image),nx/xcc,ny/ycc, /INTERP, /CENTER,cubic=-0.5)  else $
	 cgzimage, congrid(iris_histo_opt(image),nx/xcc,ny/ycc, /INTERP, /CENTER,cubic=-0.5)
 endif
 if nohist then begin
	 if not actual then if not zm then $
	 tvscl, congrid(image,nx/xcc,ny/ycc,/INTERP, /CENTER, cubic=-0.5), /nan  else $
		 cgzimage, congrid(image,nx/xcc,ny/ycc, /INTERP, /CENTER,cubic=-0.5)
 endif
 if nohist then begin
...
if ocontour eq 1 then begin
 ...
 if actual then imagei = congrid(iris_histo_opt(image),nx/xcc,ny/ycc) else $
	 imagei = congrid(iris_histo_opt(image),nx/xcc,ny/ycc, /INTERP, /CENTER,cubic=-0.5)
 mask = congrid(mask,nx/xcc,ny/ycc, /INTERP, /CENTER,cubic=-0.5)
 zrg1=minmax(image,/nan)
 if nnocolor eq 1 then userlct,/full,verbose=0,coltab=nnn
 walsa_image_plot, imagei,xrange=xrg1,yrange=yrg1, nobar=nobar, zrange=zrg1, $
 ...
 if mv then begin
	 ...
	 if amcube eq 0 then begin
		 ...
		 for i=0L, nim-1 do begin
			 ...
			 if not actual then begin
				 if aref then begin
					wset, 0
					if agamma then tvscl, GmaScl(congrid(iris_histo_opt(im),nxt/xcc,nyt/ycc,/INTERP, /CENTER, $
						cubic=-0.5),gamma=gamma), /nan  else $
				 	tvscl, congrid(iris_histo_opt(im),nx/xcc,ny/ycc, /INTERP, /CENTER,cubic=-0.5), /nan 
					...
					if atiffsavedir then $
				 	...
				 	wset, 2
					if agamma then tvscl, GmaScl(congrid(iris_histo_opt(imr),nxt/xcc,nyt/ycc,/INTERP, /CENTER, $
						cubic=-0.5),gamma=gamma), /nan  else $
					tvscl, congrid(iris_histo_opt(imr),nx/xcc,ny/ycc, /INTERP, /CENTER,cubic=-0.5), /nan 
					if atiffsavedir then $
	 ...
	 if amcube eq 1 then begin
		 ...
		 if n_elements(mcube) eq 0 then mcube=[-1,-1,0]
		 ...
		 if n_elements(mcube) eq 3 then begin
			...
			for mk=0, nim-1 do begin
				...
				if smt gt 1 then im = smooth(im,smt,/EDGE_TRUNCATE)
				...
				if aref then begin
					...
					imr = smooth(imr,smt,/EDGE_TRUNCATE)
				...
				if not actual then begin
					if aref then begin
						wset, 0
						if ascale then im = bytscl(im,scale[0],scale[1]) else im = bytscl(im,min(im),max(im))
						if agamma then tvscl, GmaScl(congrid(iris_histo_opt(im),nxt/xcc,nyt/ycc,/INTERP, /CENTER, $
							cubic=-0.5),gamma=gamma), /nan  else $
	   			 		tvscl, congrid(iris_histo_opt(im),nxt/xcc,nyt/ycc, /INTERP, /CENTER,cubic=-0.5), /nan 
	   			 		if atime then sjclock, time[mk], pos=[10,(nyt/ycc)-((nxt/ycc)*0.14)-10], size=(nxt/xcc)*0.14, thick=4, /dev, col=clockcolor
						wset, 2
						if ascale then imr = bytscl(imr,scale[0],scale[1]) else imr = bytscl(imr,min(imr),max(imr))
						if agamma then tvscl, GmaScl(congrid(iris_histo_opt(imr),nxt/xcc,nyt/ycc,/INTERP, /CENTER, $
							cubic=-0.5),gamma=gamma), /nan  else $
	   					tvscl, congrid(iris_histo_opt(imr),nxt/xcc,nyt/ycc, /INTERP, /CENTER,cubic=-0.5), /nan 
	   			 	endif else begin
						if ascale then im = bytscl(im,scale[0],scale[1]) else im = bytscl(im,min(im),max(im))
						if agamma then tvscl, GmaScl(congrid(iris_histo_opt(im),nxt/xcc,nyt/ycc,/INTERP, /CENTER, $
							cubic=-0.5),gamma=gamma), /nan  else $
				...
				if cb then cgColorbar, Range=[min(im),max(im)], /vertical, color=clbcolor
					...
					if (size(mask))[0] eq 3 then imask = congrid(reform(mask[*,*,mk]),nxt/xcc,nyt/ycc, /INTERP, /CENTER,cubic=-0.5)
					
				    if n_elements(xrange) eq 0 then xrg1=[0,sizeIM[0]-1] else xrg1=xrange
				    if n_elements(yrange) eq 0 then yrg1=[0,sizeIM[1]-1] else yrg1=yrange
				    !p.charsize=1.5
				    !x.thick=2  
				    !y.thick=2
				    if not actual then imagei = congrid(iris_histo_opt(im),nxt/xcc,nyt/ycc, /INTERP, /CENTER,cubic=-0.5) else $
						imagei = congrid(iris_histo_opt(im),nxt/xcc,nyt/ycc)
				    zrg1=minmax(imagei,/nan)
					; device, decomposed=1
				    if nnocolor eq 1 then userlct,/full,verbose=0,coltab=nnn; else ctload, clt
					; IRIS_LCT, 'NUV', r, g, b
					aia_lct,r,g,b,wavelnth=335,/load
					!P.Background = 255
				    walsa_image_plot, imagei,xrange=xrg1,yrange=yrg1, nobar=nobar, zrange=zrg1, $
				   		      contour=ocontour, nocolor=nnocolor, $
				   			  xtitle=xtitle, ytitle=ytitle, ztitle=ztitle,  $
				   		      ...
				   		      cutaspect=1, $ ;plot is scaled down to aspect
				              barpos=1, $ ;z-position scale (0=left, 1=top, 2=right, 3=bottom)
				   			  zlen=-0.4, distbar=0, xticklen=-0.03, yticklen=-0.03, xminor=4, yminor=4, $  ; zaxislen , colorbar distance
				   			  barthick=0, $  ; colorbar thickness
				   			  oc_im = imask, oc_color=oc_color, oc_thick=oc_thick, oc_nlevels=oc_nlevels, $
				   			  loadct_im = clt, noxval=0, zticks=6, dm=dm, ctfile=ctfile, charsize=1.7, position=[0.1,0.18,0.98,0.98]
			    
					if atextseries then $
						XYOUTS, 0.05, 0.95, ALIGNMENT=0, CHARSIZE=2., /NORMAL, strtrim(textseries[mk],2), COLOR=FSC_Color('Dodger Blue'), CHARTHICK=1
		 ...
		 if n_elements(mcube) eq 4 then begin
			...
			for mk=0, nim-1 do begin
				...
				if not actual then tvscl, congrid(iris_histo_opt(im),nxt/xcc,nyt/ycc,/INTERP, /CENTER, cubic=-0.5), /nan else $
					tvscl, congrid(iris_histo_opt(im),nxt/xcc,nyt/ycc), /nan 
	   	 	 	if cb then cgColorbar, Range=[min(im),max(im)], /vertical, color=clbcolor
				if atiffsavedir then $
					void = cgSnapshot(filename=savedir+'im'+strtrim(long(1000+mk),2), quality=100, /PNG, /NODIALOG)

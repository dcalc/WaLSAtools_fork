;; $Id: //depot/Release/ENVI53_IDL85/idl/idldir/lib/gaussian_function.pro#1 $
;;
...
on_error, 2                                     
  ...
  ;; create kernel
  for hh=0,width[7]-1>0 do begin
    h1 = (h[hh]^2)/(2*sigma[[7]]^2)[0]
    for gg=0,width[6]-1>0 do begin
      g1 = (g[gg]^2)/(2*sigma[[6]]^2)[0]
      for ff=0,width[5]-1>0 do begin
        f1 = (f[ff]^2)/(2*sigma[[5]]^2)[0]
        for ee=0,width[4]-1>0 do begin
          e1 = (e[ee]^2)/(2*sigma[[4]]^2)[0]
          for dd=0,width[3]-1>0 do begin
            d1 = (d[dd]^2)/(2*sigma[[3]]^2)[0]
            for cc=0,width[2]-1>0 do begin
              c1 = (c[cc]^2)/(2*sigma[[2]]^2)[0]
              for bb=0,width[1]-1>0 do begin
                b1 = (b[bb]^2)/(2*sigma[[1]]^2)[0]
                a1 = (a^2)/(2*sigma[0]^2)
                kernel[*,bb,cc,dd,ee,ff,gg,hh] = exp(-(a1+b1+c1+d1+e1+f1+g1+h1))
              endfor
            endfor
          endfor
        endfor
